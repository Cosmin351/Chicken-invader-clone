...
def drawChickens():
    global Dead, seconds, SSpawn, SShipHITBOXx, SShipHITBOXy, Progress2, EffectSize, PScore
...
def drawStart():
    global Dead, Progress2, ChickenBoss, Startticks2, seconds2, Asteroids, AsteroidSpwn, Wave, WaveUp, LevelLoad, seconds, SSpawn, RocketCoolDown, SShipHITBOXy, SShipHITBOXx, Chickens, RocketEffect, xRocket, yRocket
    if not Dead:
    ...
    if RocketsObject:
        for Rocket in RocketsObject:
            Rocket.draw(Ecran)
            if abs(Rocket.ypos - Rocket.posy) < 3 and abs(Rocket.xpos - Rocket.posx) < 3 or Rocket.x_speed == 0 and abs(Rocket.ypos - Rocket.posy) < 3:
                xRocket = Rocket.xpos
                yRocket = Rocket.ypos
                for ChickBoss in ChickenBoss:
...
def FileSave():
    global PScore, Empty, SaveFile, SaveFileTemp, Last
    j = 0
    Last = False
    Wrote = False
    if Pause:
        SaveFileTemp.close()
        SaveFileTemp = open(r'savetemp.txt', 'w+')
        ...
        SaveFile.close()
        SaveFile = open(r'save.txt', 'w+')
        SaveFile.close()
        SaveFile = open(r'save.txt', 'r+')
        SaveFileTemp.seek(0)
...
def Levels():
    global PlayerLevel, LevelLoad, Chickens, AsteroidSpwn, ChickenBoss, Progress, Aster, BossEggs, Boss, secAster
    Progress = 0
    if PlayerLevel == 1:
        if Wave == 1:
...
BackgroundTextImage = pygame.image.load('Mainpic.png')
...
BackgroundPlayImage = pygame.image.load('BackgroundPlay.jpeg')
Background2 = pygame.transform.scale(BackgroundPlayImage, (Width, Height))
Background = Background1

# Icons
Imgbifa = pygame.image.load('Bifa.png')
...
TextGUI.append(Obiecte.Text((3*Width/100), 2*Height/100, int(1.8*Width/100), Colors.White))
...
ChickenLegz = 0
Score = 0
PScore = [Lives, Rockets, Thunders, ChickenLegz, Score]
PScore.append(Obiecte.WeaponHeat(5*Width/100, 1*Height/100, 0, Width/185))

Clock = pygame.time.Clock()
Shoot = pygame.USEREVENT + 1
CooldownTime = pygame.USEREVENT + 2
...
pygame.time.set_timer(CooldownTime, 5000)
Startticks = -1
Startticks2 = -1
Startticks3 = -1
Startticksegg = -1
...
while not Exit:
    ...
    if PScore[5].counter >= 0:
        PScore[5].counter -= 0.125
    if PScore[5].counter >35:
        Cooldown = True
