...
class Text:
    def __init__(self, posx, posy, size, color):
        ...
        self.size = size
        self.color = color
        self.Font = pygame.font.Font('Icons/Fontregular.ttf',self.size)
...
class Bullet():
    ...
    def draw(self, Ecran):
        if self.Bulletnumber != 3:
            ...
            pygame.draw.rect(Ecran, Colors.MidDarkBlue,[self.posx,  self.posy, self.Lenx, self.Leny], 0)
        if Colors.Developer:
            pygame.draw.rect(Ecran, Colors.Red,[self.posx,  self.posy, self.Lenx, self.Leny], 3)
    ...
    def addBullet(self, number):
        if number == 1:
            ...
            if self.level == 0:
                ...
                self.Bullets.append(Bullet(self.posx - 2*self.sizex/100, self.posy, 0, self.bulletspeed, self.sizex, self.sizey, 3, 0))
                self.Bullets.append(Bullet(self.posx + 2*self.sizex/100, self.posy, 0, self.bulletspeed, self.sizex, self.sizey, 3, 0))
            elif self.level <= 4:
...
class Chicken():
    def __init__(self, posx, posy, x_speed, y_speed, sizex, sizey, number, xposition, yposition, xfloat, xlimit, olimit):
        ...
        if number > 10:
            ...
            self.Img = pygame.transform.scale(Imgf, (self.Lenx, self.Leny))
            self.Health = 2000 + 5*Hlth 
            if number == 12:
                self.Health = 3500 + 5*Hlth 
        else:
...
class ChickenDrop():
    def __init__(self, posx ,posy ,sizex, sizey, number):
        ...
        self.BurgerLenx = 3*self.sizex/100
        self.BurgerLeny = 5*self.sizey/100
        self.ChickLenx = 2*self.sizex/100
        self.ChickLeny = 3*self.sizey/100
        if number == 9:
            self.number = 1
...
class Rocket():
    def __init__(self, posx, posy, sizex, sizey, xpos, ypos):
        ...
        self.x_speed = 0
        self.y_speed = 0
        if posx < 45*sizex/100:
            self.ImgR = pygame.image.load('Icons/Rocket1.png')
            self.Img = pygame.transform.scale(self.ImgR, (5*sizex/100, 7*sizey/100))
            if posy > 70*sizey/100:
                self.y_speed = -5
            else:
                self.y_speed = -3
            self.x_speed = 4
        elif posx > 55*sizex/100:
            self.ImgR = pygame.image.load('Icons/Rocket3.png')
            self.Img = pygame.transform.scale(self.ImgR, (5*sizex/100, 7*sizey/100))
            if posy > 70*sizey/100:
                self.y_speed = -5
            else:
...
class Asteroids():
    def __init__(self ,sizex ,sizey, number, directie):
        ...
        self.imgR = pygame.image.load('Icons/Chickens/Asteroid'+ str(number) + '.png')
        if directie == 1:
